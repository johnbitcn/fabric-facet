...
final class FacetNeoForgeConfigScreen extends Screen {
	...
	FacetNeoForgeConfigScreen(Screen parent) {
	...
	protected void init() {
		...
		panelHeight = panelPaddingY * 2 + TITLE_HEIGHT + titleToSectionGap + generalSectionHeight + sectionGap
				+ blockSectionHeight + sectionGap + hoverSectionHeight + doneGap + ROW_HEIGHT;
		...
		addRenderableWidget(Button.builder(enabledMessage(), button -> {
			FacetNeoForgeOutlineConfig.setEnabled(!FacetNeoForgeOutlineConfig.enabled());
			FacetNeoForgeOutline.queueOutlineChunkRebuild();
		...
		addRenderableWidget(Button.builder(hoverEnabledMessage(), button -> {
			FacetNeoForgeOutlineConfig.setHoverEnabled(!FacetNeoForgeOutlineConfig.hoverEnabled());
			button.setMessage(hoverEnabledMessage());
		}).bounds(x, rowY, controlWidth, ROW_HEIGHT).build());
		rowY += ROW_HEIGHT + ROW_GAP;
		addRenderableWidget(Button.builder(distancePathVisibleMessage(), button -> {
			FacetNeoForgeOutlineConfig.setDistancePathVisible(!FacetNeoForgeOutlineConfig.distancePathVisible());
		...
		edgeWidthSlider = new ValueSlider(x, rowY, sliderWidth, ROW_HEIGHT, "config.facet.width", 1.0, 20.0,
				FacetNeoForgeOutlineConfig.edgeWidthSetting(), value -> {
					FacetNeoForgeOutlineConfig.setEdgeWidthSetting((int) Math.round(value));
					FacetNeoForgeOutline.queueOutlineChunkRebuild();
				}, FacetNeoForgeConfigScreen::formatWholeNumber, 1.0);
		addRenderableWidget(edgeWidthSlider);
		addRenderableWidget(Button.builder(Component.translatable("config.facet.reset"), button -> {
			FacetNeoForgeOutlineConfig.setEdgeWidthSetting(2);
			FacetNeoForgeOutline.queueOutlineChunkRebuild();
			edgeWidthSlider.setActualValue(2.0);
		}).bounds(resetX, rowY, RESET_WIDTH, ROW_HEIGHT).build());

		rowY = firstRowY(hoverSectionY);
		hoverWidthSlider = new ValueSlider(x, rowY, sliderWidth, ROW_HEIGHT, "config.facet.hover_width", 1.0, 8.0,
				FacetNeoForgeOutlineConfig.hoverWidth(), value -> FacetNeoForgeOutlineConfig.setHoverWidth((float) value),
				FacetNeoForgeConfigScreen::formatHoverWidth, 1.0);
		addRenderableWidget(hoverWidthSlider);
		addRenderableWidget(Button.builder(Component.translatable("config.facet.reset"), button -> {
			FacetNeoForgeOutlineConfig.setHoverWidth(2.0f);
			hoverWidthSlider.setActualValue(2.0);
		}).bounds(resetX, rowY, RESET_WIDTH, ROW_HEIGHT).build());
		addRenderableWidget(Button.builder(Component.translatable("gui.done"), button -> onClose())
				.bounds(this.width / 2 - DONE_WIDTH / 2, y, DONE_WIDTH, ROW_HEIGHT).build());
	}
	...
	private static Component enabledMessage() {
		return toggleMessage("config.facet.outline", FacetNeoForgeOutlineConfig.enabled());
	}

	private static Component hoverEnabledMessage() {
		return toggleMessage("config.facet.hover_outline", FacetNeoForgeOutlineConfig.hoverEnabled());
	}

	private static Component distancePathVisibleMessage() {
		return toggleMessage("config.facet.distance_path", FacetNeoForgeOutlineConfig.distancePathVisible());
	}
	...
	private int firstRowY(int sectionY) { return sectionY + SECTION_HEADER_HEIGHT; }
	private int controlX() { return contentX + SECTION_INSET; }
	private int controlWidth() { return contentWidth - SECTION_INSET * 2; }
	private int sliderWidth() { return controlWidth() - RESET_WIDTH - CONTROL_GAP; }
	private int resetX() { return controlX() + sliderWidth() + CONTROL_GAP; }
	private static String formatWholeNumber(double value) { return String.format(Locale.ROOT, "%.0f", value); }
	private static String formatHoverWidth(double value) { return String.format(Locale.ROOT, "%.0f px", value); }

	private interface ValueFormatter { String format(double value); }
	...
	private static final class ValueSlider extends AbstractSliderButton {
		...
		private ValueSlider(int x, int y, int width, int height, String labelKey, double min, double max, double value,
				DoubleConsumer setter, ValueFormatter formatter, double step) {
		...
		@Override
		protected void applyValue() { setter.accept(actualValue()); }
		private double actualValue() { return snap(min + (max - min) * this.value, min, max, step); }
		private static double normalize(double value, double min, double max) { return (Math.max(min, Math.min(max, value)) - min) / (max - min); }
		private static double snap(double value, double min, double max, double step) {
			double clamped = Math.max(min, Math.min(max, value));
			return step <= 0.0 ? clamped : Math.max(min, Math.min(max, Math.round(clamped / step) * step));
		}
	}
}
